...
object JoinExample {
  def main(args: Array[String]) : Unit = {
    println(Nd4j.pad(Nd4j.ones(2, 2), Array(Array(0, 0), Array(0, 1)), Nd4j.PadMode.CONSTANT))
    val content = mutable.ArrayBuilder.make[INDArray]
    content += Nd4j.ones(1, 4)
    content += Nd4j.ones(1, 4)

    println(Nd4j.vstack(JavaConverters.asJavaCollectionConverter(content.result().toSeq).asJavaCollection))
